...
class Car extends Sprite {
  ...
  stayInOncomingLane () {
    // Keep in left lane (narrower range for 2-lane road)
    if (this.x > -0.3) {
  ...
  stayInOutgoingLane () {
    // Keep in right lane (narrower range for 2-lane road)
    if (this.x < 0.3) {
      this.position.x = this.x + COURSE_CORRECTION
    } else if (this.x > 0.7) {
      this.position.x = this.x - COURSE_CORRECTION
  ...
  static spawn (z) {
    ...
    // Position cars more centrally in their lane for 2-lane road
    // Oncoming: around -0.5 (center of left half), Outgoing: around 0.5 (center of right half)
    const x = (direction * Math.random() * 0.3) + (direction * 0.35)
